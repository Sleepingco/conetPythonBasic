...
# cv2.calcOpticalFlowFarneback() 함수를 이용하여 밀집 옵티컬플로우 계산
#  빠른 연산을 위해 입력 영상 크기를 가로, 세로 로 줄여서 계산
# 움직임 벡터 크기가 특정 임계값(e.g. 2 pixels)보다 큰 영역 안의 움직임만 고려
# 나머지 영역의 움직임은 잡음으로 취급
# 움직임이 있는 영역에서 움직임 벡터의 평균 방향 검출
# 움직임이 있는 영역에서 움직임 벡터의 x방향 성분과 y방향 성분의 평균 계산
# 평균 움직임 벡터의 크기와 방향을 계산
# 평균 움직임 벡터의 크기가 특정 임계값(e.g. 4 pixels) 보다 큰 경우만 확인
# 평균 움직임 벡터의 방향 정보를 이용
