...
out = cv2.VideoWriter('output.avi', fourcc, 30, (w, h)) # 결과 비디오 저장
...
glasses = cv2.imread('glasses.png', cv2.IMREAD_UNCHANGED) # shape(w, h, 4) bgra
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        # 눈 검출
        faceROI = frame[y:y + h // 2, x:x + w] # x= x+w, y= y+h//2 높이의 범위를 반으로 줄임 얼굴에 반 세로높이 반에서만 찾겠다.
